# 根据 https://github.com/mseitzer/pytorch-fid/blob/master/src/pytorch_fid/fid_score.py 和 https://github.com/PaddlePaddle/PaddleGAN/blob/develop/ppgan/metrics/fid.py 编写
...
from paddle_fid.inception import InceptionV3
...
def calculate_fid_given_paths(paths, batch_size, dims, num_workers=1):
    ...
    for p in paths:
        if not os.path.exists(p):
            raise FileNotFoundError('这个路径不存在: %s' % p)

    block_idx = InceptionV3.BLOCK_INDEX_BY_DIM[dims]
    model = InceptionV3([block_idx])
    premodel_path = get_weights_path_from_url(INCEPTIONV3_WEIGHT_URL)
    model.load_dict(paddle.load(premodel_path))
    model.eval()
...
def save_fid_stats(paths, batch_size, dims, num_workers=1):
    """Calculates the FID of two paths"""
    if not os.path.exists(paths[0]):
        raise FileNotFoundError('这个路径不存在: %s' % paths[0])

    if os.path.exists(paths[1]):
        raise FileExistsError('Existing output file: %s' % paths[1])
...
def main():
    ...
    if args.save_stats:
        save_fid_stats(args.path, args.batch_size, args.dims, num_workers)
        return
